...
1. The Iowa data set iowa.csv is a toy example that summarises the yield of wheat (bushels per acre) for the state of Iowa between 1930-1962. In addition to yield, year, rainfall and temperature were recorded as the main predictors of yield.
    ...
    ```{r}
    iowa.df<-read.csv("data/iowa.csv",sep=';')
    ```
    \textcolor{blue}{因为csv文件中数据用“;”分隔.}
    b. How many rows and columns does `iowa.df` have?
    \newline\textcolor{blue}{有33行，10列.}
    c. What are the names of the columns of `iowa.df`?
    \newline\textcolor{blue}{names分别是"Year" "Rain0" "Temp1" "Rain1" "Temp2" "Rain2"  "Temp3" "Rain3" "Temp4" "Yield".}
    d. What is the value of row 5, column 7 of `iowa.df`?
    \newline\textcolor{blue}{79.7}
...
vector1 <- c("5", "12", "7", "32")\textcolor{blue}{创建名为“vector1”的向量}

max(vector1)\textcolor{blue}{字符串的字典序,先比较第一个字符}

sort(vector1)\textcolor{blue}{字符串的字典序，先排第一个字符}

sum(vector1)\textcolor{blue}{字符串不能求和}
    
  b. For the next series of commands, either explain their results, or why they should produce errors.

vector2 <- c("5",7,12)\textcolor{blue}{创建名为“vector1”的向量,全都是字符串}

vector2[2] + vector2[3]\textcolor{blue}{字符串不能相加}
\newline

dataframe3 <- data.frame(z1="5",z2=7,z3=12)  \textcolor{blue}{1行3列，每列的name分别是“z1”,“z2”,“z3”.}

dataframe3[1,2] + dataframe3[1,3]  \textcolor{blue}{第1行第2列的7加第1行第3列的12得19.}
\newline

list4 <- list(z1="6", z2=42, z3="49", z4=126)  \textcolor{blue}{list中可以有不同的type}

list4[[2]]+list4[[4]]  \textcolor{blue}{list4[[2]]+list4[[4]]=42+126=168}

list4[2]+list4[4]  \textcolor{blue}{list4[2]和list4[4]也是list，不能相加.}
...
3. Working with functions and operators.
    ...
    \newline\textcolor{blue}{seq(1,10000,by=372)}
    \newline\textcolor{blue}{seq(1,10000,length.out=50)}
    ...
    \newline\textcolor{blue}{rep(1:3, times=3)把整个向量重复三遍，得 1 2 3 1 2 3 1 2 3.}
    \newline\textcolor{blue}{rep(1:3, each=3)重复每个元素，得 1 1 1 2 2 2 3 3 3.}
...
```{r}
data("orings")
no=orings[c(1,2,4,11,13),]
ggplot(data=no)+geom_point(mapping= aes(x=Temperature,y=Total))
ggplot(data=orings)+geom_point(mapping= aes(x=Temperature,y=Total))
```
